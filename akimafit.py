...
class signal():
	def __init__(self,plik,tcol,mcol,ercol,period,k,n,name='',sc=None,hjd0=None,flux=True,ph=True,tomean=False,ps=0.,downlimit=-500.,uplimit=500,template = []):
		self.plik = plik
		self.tcol=tcol
		self.mcol=mcol
		self.ercol=ercol
	...
	def readtemplate(self,template_file):
		plik = os.popen('cat '+self.template_file).read().split('\n')[:-1]
		for linia in plik:
			if '#' not in linia:
				...
				self.templ_x.append(float(dane[tc]))
				self.templ_y.append(float(dane[mc]))
